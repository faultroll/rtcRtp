...
#include <signal.h>
#include <pthread.h>
static void *__do_event(void *args)
{
    int ret;
    rtsp_demo_handle demo = (rtsp_demo_handle)args;
...
int rtsp_set_audio(rtsp_session_handle session, int codec_id, const uint8_t *codec_data, int data_len)
{
    ...
    switch (codec_id) {
        ...
        case RTSP_CODEC_ID_AUDIO_G7221:
    ...
    if (codec_data && data_len > 0) {
        switch (codec_id) {
            case RTSP_CODEC_ID_AUDIO_G7221:
            case RTSP_CODEC_ID_AUDIO_G726:
                if (rtsp_codec_data_parse_from_user_g72x(codec_data, data_len, &s->acodec_data.g72x) <= 0) {
                    warn("parse codec_data failed\n");
                    break;
                }
...
static int build_simple_sdp(struct rtsp_session *s, const char *uri, char *sdpbuf, int maxlen)
{
    ...
    if (s->acodec_id != RTSP_CODEC_ID_NONE) {
        switch (s->acodec_id) {
            ...
            case RTSP_CODEC_ID_AUDIO_G7221:
            case RTSP_CODEC_ID_AUDIO_G726:
                p += rtsp_build_sdp_media_attr_g72x(ARTP_PT_ID, s->artpe.sample_rate, &s->acodec_data.g72x, p, maxlen - (p - sdpbuf));
                break;
            case RTSP_CODEC_ID_AUDIO_AAC:
...
int rtsp_do_event(rtsp_demo_handle demo)
{
    ...
    while (cc) { //exist rtsp_client_connection
        struct rtsp_client_connection *cc1 = cc;
        struct rtsp_session *s = cc1->session; //rtsp session
        struct rtp_connection *vrtp = cc1->vrtp; //rtp video
        struct rtp_connection *artp = cc1->artp; //rtp audio
        cc = TAILQ_NEXT(cc, demo_entry); //find rtsp_client_connection
...
int rtsp_tx_audio(rtsp_session_handle session, const uint8_t *frame, int len, uint64_t ts)
{
    ...
    switch (s->acodec_id) {
        ...
        case RTSP_CODEC_ID_AUDIO_G7221:
        case RTSP_CODEC_ID_AUDIO_G726:
            count = rtp_enc_g72x(&s->artpe, frame, len, ts, packets, pktsizs);
            if (count <= 0) {
                err("rtp_enc_g72x ret = %d\n", count);
                return -1;
            }
            break;
        case RTSP_CODEC_ID_AUDIO_AAC:
